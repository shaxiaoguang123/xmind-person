...
The Markdown-domain section established in T01 must preserve at least:
...
A Section owns Markdown nodes after its heading until the next heading node in document order. That body is serialized/sliced from the original source without including any later heading or its descendants.
...
This rule prevents Markdown Cards from duplicating child-section content.
...
When mdast supplies a reliable `position`, T01 preserves source location for the section heading and/or owned section range. At minimum the model can retain:
...
Offsets may also be retained when provided because they enable loss-minimizing source slicing. Line/column values MUST NOT be manually recomputed when AST positions are available.
...
T01 introduces an explicit Stable Node ID mapping boundary:

- On first projection, sections without an existing mapping receive opaque generated IDs (for example UUIDs).
- On re-projection when a previous mapping is supplied, known sections reuse their previous IDs.
- Title/body/style/layout edits do not inherently create a new identity.
